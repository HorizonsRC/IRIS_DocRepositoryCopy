...
Public Class CRC
    ...
    Private m_Algorithm As CRCAlgorithms
    ...
    Private Shared crc32Table() As Long

    'Public Sub New()
    '    ' This is the official polynomial used by CRC32 in PKZip. Often the polynomial is shown reversed (04C11DB7).
    '    Dim dwPolynomial As Long = &HEDB88320
    '    Dim i As Integer
    '    Dim j As Integer
    ...
    'Public Function GetCRC32(Buffer) As Long
    ...
    'CRC Checksum Class
    '------------------------------------------------------------------------------------------------------------------------------------------------
    'A very fast solution to calculate the CRC Checksum (at the moment CRC16 and CRC32 values) with the help of some pre-compiled assembler code
    '(c) 2000, Fredrik Qvarfort
    '------------------------------------------------------------------------------------------------------------------------------------------------
    Public Function AddBytes(ByteArray() As Byte) As Object

        Dim ByteSize As Long

        'We need to add a simple error trapping here because if the bytearray is not dimensioned we want it to just skip the assembler code call below
        Try
            'Precalculate the size of the byte array
            ByteSize = UBound(ByteArray) - LBound(ByteArray) + 1

            'No error trapping needed, if something goes bad below something is definitely fishy with your computer
            'Run the pre-compiled assembler code for the current selected algorithm
            Select Case m_Algorithm
                Case CRCAlgorithms.CRC16
                    Call CallWindowProc(VarPtr(m_CRC16Asm(0)), VarPtr(m_CRC16), VarPtr(ByteArray(LBound(ByteArray))), VarPtr(m_CRC16Table(0)), ByteSize)
                Case CRCAlgorithms.CRC32
    ...
    Public Property Algorithm() As CRCAlgorithms
        Get
            Return m_Algorithm
        End Get
        Set(value As CRCAlgorithms)
            'Set the new algorithm
            m_Algorithm = value
            'Make sure we have initialized the current selected algorithm
            Select Case m_Algorithm
                Case CRCAlgorithms.CRC16
            ...
            End Select
            'Make sure we reset the data of the current selected algorithm
            Clear()
        End Set
    End Property

    Public Function CalculateBytes(ByteArray() As Byte) As Object

        'Reset the current CRC calculation
        Clear()

        'Calculate the CRC from the bytearray and return the current CRC value
        CalculateBytes = AddBytes(ByteArray)

    End Function

    Public Function CalculateFile(Filename As String) As Object
        Dim ByteArray() As Byte

        'Make sure the file contains data to avoid errors later below
        If Not IO.File.Exists(Filename) OrElse My.Computer.FileSystem.GetFileInfo(Filename).Length = 0 Then
            Return Nothing
        Else
            ByteArray = IO.File.ReadAllBytes(Filename)
            'Now send the bytearray to the function that can calculate a CRC from it
            Return CalculateBytes(ByteArray)
        End If
    End Function
    ...
    Private Sub InitializeCRC16()
        ...
        'Create the fixed lookup-table, this is calculated because it won't take long and is only done once
        For i = 0 To 255
            k = i * 256
            CRC = 0
            For j = 0 To 7
    ...
    End Sub
    Public Sub Clear()
        'Here can be sloppy and reset both crc variables (this procedure will be more advanced when adding more checksums algorithms..)
        m_CRC16 = 0
        m_CRC32 = &HFFFFFFFF
    End Sub
