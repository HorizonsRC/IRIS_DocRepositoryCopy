...
#Region "Form Load/Unload"
    ...
    Private Sub frmMain_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If WS = FormWindowState.Normal Then
            ...
            'Me.Width = GetSetting("IRIS_DocumentMigration", "Settings", "frmMain.Width", Me.Width)
            'Me.Height = GetSetting("IRIS_DocumentMigration", "Settings", "frmMain.Height", Me.Height)
        Else
            Me.WindowState = WS
        End If
        ...
        If My.Settings.IRIS_Server.Length > 0 Then
            ...
            cmbFileMigrationID.DataSource = GetMigrationIDList()
...
#Region "Shared Methods"
    ...
    Private Function GetMigrationIDList() As ArrayList
        Dim dbList As New ArrayList
        ...
        If rdr.HasRows Then
            Do While rdr.Read
                dbList.Add(rdr("FileMigrationID"))
            Loop
        End If
        rdr.Close()
        rdr = Nothing
...
#Region "Migration Process Buttons"
    ...
    Private Sub cmdPopulateFolderTable_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdPopulateFolderTable.Click
        ...
        Dim myFolderList As String = wks.ExecuteScalar(SQL) 'Get a comma-delimited list of folders to be included in the recursive scan
    ...
    Private Function GetSQL_DocumentMapping() As String
        ...
        SQL = SQL & vbCrLf & "                CASE"
        SQL = SQL & vbCrLf & "                WHEN FolderName LIKE 'ATH-%'                    THEN 'Regimes'"
        ...
        SQL = SQL & vbCrLf & "                WHEN FolderName LIKE '%compliance%'             THEN 'Regimes'"
        ...
        SQL = SQL & vbCrLf & "                        CAST(REPLACE(REPLACE(FolderName,'-','/'),'ATH/','ATH-') AS VARCHAR(50))     ConsentNumber,"
        ...
        SQL = SQL & vbCrLf & "                        FP.ConsentNumber,"
        ...
        SQL = SQL & vbCrLf & "    CREATE INDEX idx_ConsentNumber ON #FolderPath(ConsentNumber)"
        SQL = SQL & vbCrLf & "END"
        SQL = SQL & vbCrLf & "------------------------------------------------------------------------------------------------------------------------------------------------"
        SQL = SQL & vbCrLf & "--  Authorisation - IRISObjectID"
        SQL = SQL & vbCrLf & "------------------------------------------------------------------------------------------------------------------------------------------------"
        SQL = SQL & vbCrLf & "IF OBJECT_ID(N'tempdb..#GetIRISObjectID',N'U') IS NULL"
        SQL = SQL & vbCrLf & "BEGIN"
        SQL = SQL & vbCrLf & "    SELECT      ATH.IRISObjectID,"
        SQL = SQL & vbCrLf & "                OID.OtherIdentifierText                                                                     LegacyID,"
        SQL = SQL & vbCrLf & "                ATO.BusinessID                                                                              AuthorisationIRISID"
        SQL = SQL & vbCrLf & "    INTO        #GetIRISObjectID"
        SQL = SQL & vbCrLf & "    FROM        IRIS[ENV].dbo.Authorisation                                                                  ATH"
        SQL = SQL & vbCrLf & "    LEFT JOIN   IRIS[ENV].dbo.IRISObject                                                                     ATO"
        SQL = SQL & vbCrLf & "    ON          ATO.ID = ATH.IRISObjectID"
        SQL = SQL & vbCrLf & "    LEFT JOIN  (SELECT      IRISObjectID,"
        SQL = SQL & vbCrLf & "                            OtherIdentifierText"
        SQL = SQL & vbCrLf & "                FROM        IRIS[ENV].dbo.OtherIdentifier"
        SQL = SQL & vbCrLf & "                WHERE       IsDeleted = 0"
        SQL = SQL & vbCrLf & "                AND         IdentifierContextID = 427)                                                      OID"
        SQL = SQL & vbCrLf & "    ON          OID.IRISObjectID = ATH.IRISObjectID"
        SQL = SQL & vbCrLf & "    WHERE       ATH.IsDeleted = 0"
        SQL = SQL & vbCrLf & ""
        ...
        SQL = SQL & vbCrLf & "    CREATE INDEX idx_AuthorisationID ON #GetIRISObjectID(AuthorisationIRISID)"
        SQL = SQL & vbCrLf & "END"
        SQL = SQL & vbCrLf & "------------------------------------------------------------------------------------------------------------------------------------------------"
        ...
        SQL = SQL & vbCrLf & "            FP.TargetFolder+'\Regimes\'+MAIN.RegimeIRISID COLLATE DATABASE_DEFAULT+'\Legacy Monitoring Activities'                          TargetFolder"
        SQL = SQL & vbCrLf & "FROM       (SELECT      DISTINCT"
        SQL = SQL & vbCrLf & "                        FILES.RegimeIRISID,"
        SQL = SQL & vbCrLf & "                        FILES.MinFolderID,"
        SQL = SQL & vbCrLf & "                        FILES.FileName,"
        SQL = SQL & vbCrLf & "                        FILES.FileSize,"
        SQL = SQL & vbCrLf & "                        FILES.Checksum,"
        SQL = SQL & vbCrLf & "                        COUNT(*)        OVER(PARTITION BY RegimeIRISID,FileName)                                                            CopyCount,"
        SQL = SQL & vbCrLf & "                        RANK()          OVER(PARTITION BY RegimeIRISID,FileName"
        SQL = SQL & vbCrLf & "                                                ORDER BY     SourceFolder)                                                                  VersionNumber"
        SQL = SQL & vbCrLf & "            FROM       (SELECT      DISTINCT"
        SQL = SQL & vbCrLf & "                                    RGO.BusinessID                                                                                          RegimeIRISID,"
        ...
        SQL = SQL & vbCrLf & "                                    MIN(FP.FolderID)    OVER(PARTITION BY RGO.BusinessID,FL.FileName,FL.FileSize,FL.[Checksum])             MinFolderID,"
        SQL = SQL & vbCrLf & "                                    RANK()              OVER(PARTITION BY RGO.BusinessID,FL.FileName,FL.FileSize,FL.[Checksum]"
        ...
        SQL = SQL & vbCrLf & "                        ------------------------------------------------------------------------------------------------------------------------------------------------"
        SQL = SQL & vbCrLf & "                        --  Use Consent Number to identify the IRIS Authorisation via the OtherIdentifier mapping"
        SQL = SQL & vbCrLf & "                        ------------------------------------------------------------------------------------------------------------------------------------------------"
        SQL = SQL & vbCrLf & "                        LEFT JOIN   #GetIRISObjectID                                                                                        LEG"
        SQL = SQL & vbCrLf & "                        ON          LEG.LegacyID            = FP.ConsentNumber COLLATE DATABASE_DEFAULT"
        SQL = SQL & vbCrLf & "                        OR          LEG.AuthorisationIRISID = FP.ConsentNumber COLLATE DATABASE_DEFAULT"
        SQL = SQL & vbCrLf & "                        ------------------------------------------------------------------------------------------------------------------------------------------------"
        ...
        SQL = SQL & vbCrLf & "                        ------------------------------------------------------------------------------------------------------------------------------------------------"
        SQL = SQL & vbCrLf & "                        OUTER APPLY IRIS[ENV].dbo.GetSimpleLinksWithoutSecurity(LEG.IRISObjectID)                                           AOR"
        SQL = SQL & vbCrLf & "                        LEFT JOIN   IRIS[ENV].dbo.IRISObject                                                                                RGO"
        SQL = SQL & vbCrLf & "                        ON          RGO.ID = AOR.LinkedIRISObjectID"
        SQL = SQL & vbCrLf & "                        AND         RGO.ObjectTypeID = 12"
        SQL = SQL & vbCrLf & "                        ------------------------------------------------------------------------------------------------------------------------------------------------"
        SQL = SQL & vbCrLf & "                        --  Get the associated IRIS object so we can work out the Regime Activity Type (RC2) and SubType (RC3)"
        SQL = SQL & vbCrLf & "                        --  Also, identify the associated Regime Activity (the Regime Activity connects to the Regime via a Regime Activity Schedule)"
        SQL = SQL & vbCrLf & "                        ------------------------------------------------------------------------------------------------------------------------------------------------"
        SQL = SQL & vbCrLf & "                        LEFT JOIN  (SELECT      RAC.IRISObjectID                                                                            RegimeActivityIRISObjectID,"
        SQL = SQL & vbCrLf & "                                                RAC.ID                                                                                      RegimeActivityID,"
        SQL = SQL & vbCrLf & "                                                RAO.BusinessID                                                                              RegimeActivityIRISID,"
        SQL = SQL & vbCrLf & "                                                RAS.ID                                                                                      RegimeActivityScheduleID,"
        SQL = SQL & vbCrLf & "                                                RAS.RegimeID"
        SQL = SQL & vbCrLf & "                                    FROM        IRIS[ENV].dbo.RegimeActivity                                                                RAC"
        SQL = SQL & vbCrLf & "                                    LEFT JOIN   IRIS[ENV].dbo.IRISObject                                                                    RAO"
        SQL = SQL & vbCrLf & "                                    ON          RAO.ID = RAC.IRISObjectID"
        SQL = SQL & vbCrLf & "                                    LEFT JOIN   IRIS[ENV].dbo.ReferenceDataValue                                                            SC2"
        SQL = SQL & vbCrLf & "                                    ON          SC2.ID = RAO.SubClass2ID"
        SQL = SQL & vbCrLf & "                                    LEFT JOIN   IRIS[ENV].dbo.ReferenceDataValue                                                            SC3"
        SQL = SQL & vbCrLf & "                                    ON          SC3.ID = RAO.SubClass3ID"
        SQL = SQL & vbCrLf & "                                    LEFT JOIN   IRIS[ENV].dbo.RegimeActivitySchedule                                                        RAS"
        SQL = SQL & vbCrLf & "                                    ON          RAS.ID = RAC.RegimeActivityScheduleID"
        SQL = SQL & vbCrLf & "                                    WHERE       RAO.ObjectTypeID = 13"
        SQL = SQL & vbCrLf & "                                    AND         SC2.DisplayValue = 'Historic'"
        SQL = SQL & vbCrLf & "                                    AND         SC3.DisplayValue = 'Legacy Monitoring Activities')                                          HRA"
        SQL = SQL & vbCrLf & "                        ON          HRA.RegimeID = RGO.LinkID"
        SQL = SQL & vbCrLf & "                        ------------------------------------------------------------------------------------------------------------------------------------------------"
        SQL = SQL & vbCrLf & "                        WHERE       FL.FileMigrationID = @FileMigrationID"
        SQL = SQL & vbCrLf & "                        AND         FL.Migrated = 0"
        SQL = SQL & vbCrLf & "                        AND         FP.FileCount > 0"
        SQL = SQL & vbCrLf & "                        AND         FL.FileName IS NOT NULL"
        SQL = SQL & vbCrLf & "                        AND         FP.ObjectType = 'Regimes'"
        SQL = SQL & vbCrLf & "                        AND         AOR.LinkedAsRelationshipType = 'Regime Subject')                                                        FILES"
        ...
        SQL = SQL & vbCrLf & "--WHERE       MAIN.RegimeIRISID IN ('RGM-1987502073','RGM-1990502125')"
        SQL = SQL & vbCrLf & "ORDER BY    RegimeIRISID,"
    ...
    Private Function GetSQL_IdentifyDuplicates() As String
        ...
        SQL = SQL & vbCrLf & "------------------------------------------------------------------------------------------------------------------------------------------------"
        SQL = SQL & vbCrLf & "DECLARE @MigrationID INTEGER = " & My.Settings.MigrationID
        SQL = SQL & vbCrLf & "DECLARE @FileMigrationID VARCHAR(50) = '" & My.Settings.FileMigrationID & "';"
        SQL = SQL & vbCrLf & "DECLARE @GetLegacy BIT = " & IIf(My.Settings.SourceRootFolder Like "*E\RM\04\00*", "1", "0")
        SQL = SQL & vbCrLf & "------------------------------------------------------------------------------------------------------------------------------------------------"
        ...
        SQL = SQL & vbCrLf & "BEGIN"
        SQL = SQL & vbCrLf & "    --------------------------------------------------------------------------------------------------------------------------------------------"
        SQL = SQL & vbCrLf & "    SELECT      RGM.IRISObjectID                                                                                    RegimeIRISObjectID,"
        SQL = SQL & vbCrLf & "                RGO.BusinessID                                                                                      RegimeIRISID,"
        SQL = SQL & vbCrLf & "                IIF(@GetLegacy = 1,LEG.LegacyID,ATO.BusinessID)                                                     TextualID"
        SQL = SQL & vbCrLf & "    --------------------------------------------------------------------------------------------------------------------------------------------"
        SQL = SQL & vbCrLf & "    INTO        #IRIS_Lookup"
        SQL = SQL & vbCrLf & "    --------------------------------------------------------------------------------------------------------------------------------------------"
        SQL = SQL & vbCrLf & "    FROM       (SELECT      DISTINCT"
        SQL = SQL & vbCrLf & "                            RegimeID,"
        SQL = SQL & vbCrLf & "                            AuthorisationIRISID"
        SQL = SQL & vbCrLf & "                FROM        HRC.MapRegime"
        SQL = SQL & vbCrLf & "                WHERE       MigrationID = @MigrationID"
        SQL = SQL & vbCrLf & "                AND         Migrate = 1)                                                                            MAP"
        SQL = SQL & vbCrLf & "    LEFT JOIN   IRIS[ENV].dbo.IRISObject                                                                              ATO"
        SQL = SQL & vbCrLf & "    ON          ATO.BusinessID = MAP.AuthorisationIRISID"
        SQL = SQL & vbCrLf & "    LEFT JOIN   IRIS[ENV].dbo.Regime                                                                                  RGM"
        SQL = SQL & vbCrLf & "    ON          RGM.ID = MAP.RegimeID"
        SQL = SQL & vbCrLf & "    LEFT JOIN   IRIS[ENV].dbo.IRISObject                                                                              RGO"
        SQL = SQL & vbCrLf & "    ON          RGO.ID = RGM.IRISObjectID"
        SQL = SQL & vbCrLf & "    LEFT JOIN  (SELECT      IRISObjectID,"
        SQL = SQL & vbCrLf & "                            CAST(OtherIdentifierText AS NVARCHAR(100))                                              LegacyID"
        SQL = SQL & vbCrLf & "                FROM        IRIS[ENV].dbo.OtherIdentifier"
        SQL = SQL & vbCrLf & "                WHERE       IsDeleted = 0"
        SQL = SQL & vbCrLf & "                AND         IdentifierContextID = 427)                                                              LEG"
        SQL = SQL & vbCrLf & "    ON          LEG.IRISObjectID = ATO.ID"
        SQL = SQL & vbCrLf & "    ORDER BY    1"
        SQL = SQL & vbCrLf & "    --------------------------------------------------------------------------------------------------------------------------------------------"
        SQL = SQL & vbCrLf & "    CREATE INDEX idx_TextualID ON #IRIS_Lookup(TextualID)"
        SQL = SQL & vbCrLf & "    --------------------------------------------------------------------------------------------------------------------------------------------"
        SQL = SQL & vbCrLf & "END"
        SQL = SQL & vbCrLf & "------------------------------------------------------------------------------------------------------------------------------------------------"
        ...
        SQL = SQL & vbCrLf & "    --------------------------------------------------------------------------------------------------------------------------------------------"
        SQL = SQL & vbCrLf & "    AS     (SELECT      LKP.RegimeIRISID,"
        SQL = SQL & vbCrLf & "                        FLD.FolderUID,"
        SQL = SQL & vbCrLf & "                        FLD.FolderID,"
        SQL = SQL & vbCrLf & "                        FLD.ParentFolderID,"
        SQL = SQL & vbCrLf & "                        FLD.TextualID,"
        SQL = SQL & vbCrLf & "                        FLD.FolderName,"
        SQL = SQL & vbCrLf & "                        FLD.FolderPath,"
        SQL = SQL & vbCrLf & "                        FLD.ErrorText"
        SQL = SQL & vbCrLf & "            FROM       (SELECT      FLD.ID                                                                          FolderUID,"
        SQL = SQL & vbCrLf & "                                    FLD.FolderID,"
        SQL = SQL & vbCrLf & "                                    FLD.ParentFolderID,"
        SQL = SQL & vbCrLf & "                                    CASE"
        SQL = SQL & vbCrLf & "                                    WHEN PATINDEX('[0-9]%',FLD.FolderName)=1                             THEN REPLACE(FLD.FolderName,'-','/')"
        SQL = SQL & vbCrLf & "                                    WHEN LEFT(FLD.FolderName+'  ' ,2) IN ('CR','MC','MW','NU','PA','RT') THEN RTRIM(LTRIM(ITM.Item))"
        SQL = SQL & vbCrLf & "                                    WHEN LEFT(FLD.FolderName+'   ',3) IN ('ATH','APP','RGM','PGM')       THEN RTRIM(LTRIM(ITM.Item))"
        SQL = SQL & vbCrLf & "                                    ELSE NULL END                                                                   TextualID,"
        SQL = SQL & vbCrLf & "                                    FLD.FolderName,"
        ...
        SQL = SQL & vbCrLf & "            ON          LKP.TextualID = FLD.TextualID"
        SQL = SQL & vbCrLf & "            UNION ALL"
        SQL = SQL & vbCrLf & "            SELECT      A.RegimeIRISID,"
        SQL = SQL & vbCrLf & "                        B.ID                                                                                        FolderUID,"
        SQL = SQL & vbCrLf & "                        B.FolderID,"
        SQL = SQL & vbCrLf & "                        B.ParentFolderID,"
        SQL = SQL & vbCrLf & "                        A.TextualID,"
        SQL = SQL & vbCrLf & "                        B.FolderName,"
        ...
        SQL = SQL & vbCrLf & "    CREATE INDEX idx_TextualID  ON #Folders(TextualID)"
        SQL = SQL & vbCrLf & "    --------------------------------------------------------------------------------------------------------------------------------------------"
        SQL = SQL & vbCrLf & "END"
        SQL = SQL & vbCrLf & "------------------------------------------------------------------------------------------------------------------------------------------------"
        ...
        SQL = SQL & vbCrLf & "    AND         R.RegimeIRISID IS NOT NULL"
        SQL = SQL & vbCrLf & "    --------------------------------------------------------------------------------------------------------------------------------------------"
        SQL = SQL & vbCrLf & "    CREATE INDEX idx_RegimeID ON #AllFiles(RegimeIRISID,FileName,FileSize)"
        SQL = SQL & vbCrLf & "    CREATE INDEX idx_LegacyID ON #AllFiles(TextualID)"
        SQL = SQL & vbCrLf & "    --------------------------------------------------------------------------------------------------------------------------------------------"
        SQL = SQL & vbCrLf & "END"
        SQL = SQL & vbCrLf & "------------------------------------------------------------------------------------------------------------------------------------------------"
        ...
        SQL = SQL & vbCrLf & "            GROUP BY    RegimeIRISID,"
        SQL = SQL & vbCrLf & "                        FileName,"
        SQL = SQL & vbCrLf & "                        FileSize)                                                                               MAIN"
        SQL = SQL & vbCrLf & "LEFT JOIN   #AllFiles                                                                                           RF"
        SQL = SQL & vbCrLf & "ON          RF.RegimeIRISID = MAIN.RegimeIRISID"
